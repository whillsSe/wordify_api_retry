...
public class ContextDaoImpl implements ContextDao{
    ...
    public ContextDto getContext(ContextRetrievalPayload query, Connection conn) {
        ...
        StringBuilder builder = new StringBuilder("WITH sorted_keys AS (SELECT w.id AS word_id,w.word AS word,p.id AS phonetic_id,p.phonetic AS phonetic,sub.id AS definition_id,sub.collector_id AS collector_id,d.user_id AS user_id,ROW_NUMBER() OVER (ORDER BY p.phonetic,w.word) AS row_num FROM definitions d JOIN (");
